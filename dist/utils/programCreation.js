...
exports.handleProgramCreationMessage = exports.startProgramCreation = void 0;
const UserData_1 = require("../types/UserData");
const training_1 = require("../types/training");
// Функция для преобразования ПМ в проценты то есть 6ПМ это 85%
function pmToPercentage(pm) {
    return training_1.repMaxTable[pm] || 85;
}
// Генерация текста программы для вывода пользователю
function generateProgramText(programData) {
...
async function startProgramCreation(bot, chatId) {
    (0, UserData_1.setUserData)(chatId, {
        programData: { weeks: [] },
        currentStep: "days",
    });
    await bot.sendMessage(chatId, "🏋️ *Создадим тебе план на основе твоей программы!*\n\n" +
        "📅 *В какие дни тренируешься?*\n" +
        "▫️ Пример: пн, ср, пт\n" +
        "▫️ Или: вт, чт, сб\n\n" +
        "(Укажи дни через запятую)", { parse_mode: "Markdown" });
}
exports.startProgramCreation = startProgramCreation;
// Обработка шагов программы
async function handleProgramCreationMessage(bot, chatId, text) {
    ...
    switch (step) {
        case "days": {
            ...
            if (!days.length) {
                await bot.sendMessage(chatId, "❌ *Не понял дни*\n\n" +
                    "💡 *Попробуй так:*\n" +
                    "▫️ пн, ср, пт\n" +
                    "▫️ вт, чт, сб", { parse_mode: "Markdown" });
                return;
            }
            (0, UserData_1.setUserData)(chatId, {
                tempDays: days,
            ...
            await bot.sendMessage(chatId, "📝 *Теперь укажи нагрузки для каждого дня:*\n\n" +
                "💡 *Какой ПМ указан в твоей программе?(6ПМ,2ПМ)*\n" +
                "▫️ Примеры форматов тренировок:\n" +
                "   - 6ПМ 3×5 (для 6ПМ)\n" +
                "   - 80% 4×3 (для процентов)\n" +
                "   - 5ПМ ×8 (один подход)\n\n" +
                `💡 *Указывая такие строки, ты задаешь нагрузки для дней недели:* ${days.join(', ')}`, { parse_mode: "Markdown" });
            return;
        }
        case "dayPercentSetsReps": {
            ...
            if (!configs.length) {
                await bot.sendMessage(chatId, "❌ *Не понял \n\n" +
                    ...
                    "▫️ 80% 4×3\n" +
                    "▫️ 5ПМ ×8\n\n" +
                    `📌 *Нужно ${u.tempDays?.length || 3} строки для дней:* ${u.tempDays?.join(', ') || 'пн, ср, пт'}`, { parse_mode: "Markdown" });
                return;
            ...
            if (u.tempDays && configs.length !== u.tempDays.length) {
                await bot.sendMessage(chatId, `❌ *Количество не совпадает!*\n\n` +
                    ...
                    `▫️ ${u.tempDays[2]}: 6ПМ ×5`, { parse_mode: "Markdown" });
                return;
            }
            (0, UserData_1.setUserData)(chatId, {
            ...
            await bot.sendMessage(chatId, "💪 *Отлично! Теперь укажи свой 1ПМ:*\n\n" +
                "▫️ Пример: 50\n" +
                "▫️ Или: 120\n\n" +
                "💡 *1ПМ* — максимальный вес, который можешь поднять 1 раз", { parse_mode: "Markdown" });
            return;
        }
        case "dayPercentSetsReps": {
            const lines = text.split(/\n+/).map((s) => s.trim()).filter(Boolean);
            ...
            if (!configs.length) {
                await bot.sendMessage(chatId, "❌ *Не понял \n\n" +
                    ...
                    "▫️ 6ПМ 3x5 = 6ПМ 3×5\n" +
                    "▫️ 80% 4x3 = 80% 4×3", { parse_mode: "Markdown" });
                return;
            }
            // Проверяем, что количество конфигов совпадает с количеством дней
            if (u.tempDays && configs.length !== u.tempDays.length) {
        ...
        case "maxWeight": {
            ...
            if (isNaN(max) || max <= 0) {
                await bot.sendMessage(chatId, "❌ *Нужно число!*\n\n" +
                    "💡 *Пример:* 50 или 120", { parse_mode: "Markdown" });
                return;
            }
            const pd = u.programData || { weeks: [] };
            ...
            await bot.sendMessage(chatId, "❓ *Добавить ещё одну неделю?*\n\n" +
                "▫️ *Да* — добавлю новую неделю\n" +
                "▫️ *Нет* — завершаю программу", { parse_mode: "Markdown" });
            return;
        }
        case "addWeek": {
            if (text.toLowerCase().startsWith("д")) {
                (0, UserData_1.setUserData)(chatId, { currentStep: "days" });
                await bot.sendMessage(chatId, "📅 *Укажи дни для следующей недели:*\n\n" +
                    "💡 Пример: пн, ср, пт", { parse_mode: "Markdown" });
            }
            else {
                (0, UserData_1.setUserData)(chatId, { currentStep: null });
                await bot.sendMessage(chatId, "🎉 *ПРОГРАММА СОЗДАНА!*\n\n" +
                    "💪 *Удачи в тренировках родной!*", { parse_mode: "Markdown" });
            }
            return;
        }
    }
}
exports.handleProgramCreationMessage = handleProgramCreationMessage;
